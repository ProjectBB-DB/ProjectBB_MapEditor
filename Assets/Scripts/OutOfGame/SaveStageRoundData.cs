...
// 이 부분은 Editor를 위함 입니다.
// 라운드 에디터를 위한 블록 제네레이터 데이터 박스
[System.Serializable]
public class BGDforRoundEditor
{
    [Tooltip("라운드에서 사용될 블록 버튼을 넣어주세요")]
    public GameObject BlockGenerator;

    [Tooltip("해당 블록의 제공 갯수를 넣어주세요")]
    public int amount;
}
/************************************/
...
public class SaveStageRoundData : MonoBehaviour
{
    ...
    // 이전 스테입지와 다음 스테이지 체이닝을 위함
    // 이전 스테이지 라운드
    [Tooltip("이전 '스테이지' 와 '라운드'\n존재하지 않을 경우(ex. 해당 라운드가 스테이지의 첫 라운드)\n스테이지와 라운드에 '-1'을 넣어주세요")]
    public int previousStage = -1;
    public int previousRound = -1;


    [Tooltip("다음 '스테이지' 와 '라운드'\n존재하지 않을 경우(ex. 해당 라운드가 스테이지의 마지막 라운드)\n스테이지와 라운드에 '-1'을 넣어주세요")]
    // 다음 스테이지 라운드
    ...
    [SerializeField] private BGDforRoundEditor[] blockGeneratorDataForRoundData;
    // [Space(3)]
    ...
    private RoundDataStorage CreateDataStorage()
    {
        ...
        for (int i = 0; i < blockGeneratorDataForRoundData.Length; i++)
        {
            BlockGeneratorData _bgData = new BlockGeneratorData();
            _bgData.name = blockGeneratorDataForRoundData[i].BlockGenerator.name;
            _bgData.amount = blockGeneratorDataForRoundData[i].amount;
